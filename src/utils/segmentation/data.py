...
def segmentation_create_dataloaders(config):
  if config.mode == "IID+":
    ...
    elif config.dataset == "Duckietown":
      config.train_partitions = ["train"]
      config.mapping_assignment_partitions = ["train"]
      config.mapping_test_partitions = ["train"]
    else:
      raise NotImplementedError
  ...
  if "Coco" in config.dataset:
    ...
    dataloaders, mapping_assignment_dataloader, mapping_test_dataloader = \
      make_Duckietown_dataloaders(config)
  else:
    raise NotImplementedError
...
def make_Duckietown_dataloaders(config):
  dataloaders = _create_dataloaders(config, duckietown.__dict__[config.dataset])

  mapping_assignment_dataloader = \
    _create_mapping_loader(config, duckietown.__dict__[config.dataset],
                           partitions=config.mapping_assignment_partitions)

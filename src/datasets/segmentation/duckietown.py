...
class _Duckietown(data.Dataset):
    """Base class
    ...
    def __init__(self, config=None, split=None, purpose=None, preload=False):
        super(_Duckietown, self).__init__()
    ...
    def _set_files(self):
        if self.split in ['train', 'test']:
            self.files = glob.glob(os.path.join(self.root, self.split))
        else:
            raise ValueError("Invalid split name: {}".format(self.split))

    def _load_data(self, image_id):
        image = cv2.imread(image_id, cv2.IMREAD_COLOR).astype(np.uint8)
        assert (image.dtype == np.uint8)
        return image, None


class Duckietown(_Duckietown):
    def __init__(self, **kwargs):
        super(Duckietown, self).__init__(**kwargs)
